...
<div class="account-content">
	...
	<div id="login-form" class="container">
		...
		<!-- <ul class="nav navbar-nav navbar-right user-menu pull-right">
			<li>
				<a href="<?=base_url()?>candidates/all_jobs_user">Career</a>
			</li>				 
		</ul> -->	
...
<?php 
	$resignation = $this->db->get_where('resignation',array('status'=>0))->result_array();
	foreach ($resignation as $key => $value) {
		if(date('Y-m-d') > $value['noticedate']){
			$this->db->where('id', $value['id'])->update('resignation',array('status'=>1));
			$this->db->where('id', $value['employee'])->update('users',array('activated'=>0));
		}
		# code...
	}
	$termination = $this->db->get_where('termination',array('status'=>0))->result_array();
	foreach ($termination as $key => $value) {
		if(date('Y-m-d') > $value['lastdate']){
			$this->db->where('id', $value['id'])->update('termination',array('status'=>1));
			$this->db->where('id', $value['employee'])->update('users',array('activated'=>0));
		}
		# code...
	}
