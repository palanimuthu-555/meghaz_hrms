...
class Invoices extends MX_Controller
{
    public function __construct()
    {
        ...
        if(!App::is_access('menu_invoices'))
		{
    ...
    public function send_invoice($invoice_id = null)
    {
        ...
        $activity = array(
            ...
            $this->session->set_flashdata('tokbox_success', 'invoice_sent_successfully');
	...
	public function upload_invoice_logo($data)
    {
        Applib::is_demo();
    ...
    function invoice_settings(){
		if(!App::is_access('menu_settings_invoice'))
		{
		$this->session->set_flashdata('tokbox_error', lang('access_denied'));
		redirect('');
		}	
        $this->template->title(lang('invoice_settings').' - '.config_item('company_name'));
    ...
    function payment_settings(){
		if(!App::is_access('menu_settings_payment'))
		{
		$this->session->set_flashdata('tokbox_error', lang('access_denied'));
		redirect('');
		}		

        $this->template->title(lang('payment_settings').' - '.config_item('company_name'));
    ...
    function invoice_settings_update()
    {
        ...
        if(file_exists($_FILES['invoicelogo']['tmp_name']) || is_uploaded_file($_FILES['invoicelogo']['tmp_name'])) {
            $upload_response = $this->upload_invoice_logo($this->input->post());
			if ($upload_response) {
                $data = array('value' => $upload_response);
