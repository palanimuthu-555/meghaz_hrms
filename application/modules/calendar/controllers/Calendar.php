...
class Calendar extends MX_Controller {
    ...
    function __construct()
    {
		...
		if(!App::is_access('menu_calendar'))
		{
            ...
            if ($this->form_validation->run() == FALSE)
            {
               ...
               /* $start_date = Applib::date_formatter($this->input->post('start_date', TRUE));
                ...
                $data = array(
                    ...
                    'end_date'      => $end_date,
                    'from_time'     => !empty($_POST['from_time'])?date("H:i", strtotime($_POST['from_time'])):'',
                    'to_time'     => !empty($_POST['to_time'])?date("H:i", strtotime($_POST['to_time'])):'',
...
function edit_event($id = NULL)
    {
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $end_date = Applib::date_formatter($this->input->post('end_date', TRUE));
                    ...
                    // }
                     if(User::is_admin() || $added_by == User::get_id()){
                        $this->db->where('id',$id)->update('events', $data);
                    }
    ...
    // }
        ...
        if(sizeof(array_filter($data))>0)
        {
            ...
            {
                $return_array[] = array('title'=>$value['event_name'],'start'=>$value['start_date'].'T'.date('H:i:s', strtotime($value['from_time'])),'end'=>$value['start_date'].'T'.date('H:i:s', strtotime($value['to_time'])),'backgroundColor' =>$value['color']); 
            }
        } 
