				...
				<table class="table table-striped custom-table m-b-0 text-sm">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a class="float-left thumb-xs avatar text-info" data-toggle="tooltip" data-original-title="<?=User::displayName($bug->assigned_to)?>" data-toggle="tooltip" data-placement="right" title = "">
									<img src="<?=User::avatar_url($bug->assigned_to); ?>" class="rounded">
								</a>
							</td>
							<td>
								<a href="<?=base_url()?>projects/view/<?=$bug->project?>?group=bugs&view=bug&id=<?=$bug->bug_id?>">
						...
						<tr>
							<td>
								<a href="<?=base_url()?>invoices/view/<?=$invoice->inv_id?>">
									<span class="badge badge-primary"><?=$invoice->reference_no;?></span>
								</a>
							</td>
							<td>
							...
							<td>
								<span class="badge badge-danger"><?=strftime(config_item('date_format'), strtotime($invoice->due_date))?></span>
							</td>
