...
class employees extends MX_Controller {
    ...
    function index(){
        if(!App::is_access('all_employees'))
        {
 ...
 public function search_emp_profile()
    {
        $this->load->module('layouts');
        $this->load->library('template');
        ...
        $data['datatables'] = true;
        $data['form'] = TRUE;
        $data['page'] = 'search_emp_profile';
        $data['Employee'] = $this->db->select('user.*,dep.deptname,emp.city,emp.state,emp.country,emp.address,emp.pincode,emp.gender,emp.dob,emp.fullname,emp.avatar,emp.company,emp.phone,emp.doj,pos.designation')
            ->from('dgt_users user')
            ->join('dgt_departments dep', 'user.department_id=dep.deptid', 'left')
            ->join('dgt_account_details emp', 'user.id=emp.user_id', 'left')
            ->join('dgt_designation pos', 'user.designation_id=pos.id', 'left')
            ->where(array("user.role_id"=>3))
            ->like("user.username",$_POST['search'])
            ->or_like("emp.fullname",$_POST['search'])
            ->or_like("user.email",$_POST['search'])
            ->get()
            ->result();  
            // echo"<pre>"; print_r($data['Employee']); exit;
        $this->template
            ->set_layout('users')
            ->build('search_emp_profile', isset($data) ? $data : NULL);
