...
class Budget_revenues extends MX_Controller {
	...
	function __construct()
	{
		...
		if(!App::is_access('menu_budget_revenues'))
		{
	...
	function create()
	{
		...
		if($revenue_id = App::save_data('budget_revenues',$data)){
			...
			// Log activity
			$data1 = array(
				'module' => 'budget_revenues',
				'module_field_id' => $revenue_id,
				'user' => User::get_id(),
				'activity' => lang('revenue_created_successfully'),
				'icon' => 'fa-plus',
				'value1' => $cur.' '.$this->input->post('amount'),
				'value2' => $this->input->post('notes')
				);
			App::Log($data1);
