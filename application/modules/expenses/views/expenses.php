		...
		<div class="col-sm-4 col-3">
			<h4 class="page-title"><?=lang('expenses')?></h4>
				<ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><?=lang('expenses')?></li>
                </ul>
		</div>
		<div class="col-sm-8 col-9 text-right m-b-0">

			<?php if(App::is_permit('menu_expenses','create'))
			{ ?>
			<a href="<?=base_url()?>expenses/create" data-toggle="ajaxModal" title="<?=lang('create_expense')?>" class="btn btn-primary add-btn float-right"><i class="fa fa-plus"></i> <?=lang('create_expense')?></a>
			<?php
			}
			?>
	...
	<div class="row filter-row">
		<div class="col-lg-2 col-md-6 col-sm-6 col-12">  
			<div class="form-group form-focus">
				...
				<input type="text" class="form-control floating expense_submit_search" id="expenes_project" name="expenes_project">
		...
		<div class="col-lg-2 col-md-6 col-sm-6 col-12">  
			<div class="form-group form-focus">
				...
				<input type="text" class="form-control floating expense_submit_search" id="expenes_client" name="expenes_client">
				<label id="expenes_client_error" class="error display-none" for="expenes_client">Staff Shouldn't be empty</label>
			</div>
		</div>
	<?php } ?>
		<?php 
		...
		<div class="col-lg-2 col-md-6 col-sm-6 col-12">  
			<a href="javascript:void(0)" class="btn btn-primary btn-block" id="search_expenses_btn"> Search </a>  
		</div>     
	</div>

	<?php if (($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin') || ($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'superadmin')) { 

	$expenses = $this->db->query("SELECT * FROM `dgt_expenses` order by id  DESC ")->result();
	?>
		...
		<table id="table-expenses" class="table lable-table table-striped custom-table m-b-0">
			<thead>
				<tr>
					...
					<th class="col-currency text-center"><?=lang('amount')?></th>
			...
			<tr id="row_id_<?=$e->id?>" >
				...
				<td class="col-currency" style="text-align: center;">
					<strong>
					<?php
				...
				<?php 
					...
					<div class="action-label">
						<a class="btn btn-white btn-sm btn-rounded" href="javascript:void(0);">
							<i class="fa fa-dot-circle-o text-<?=$label_color?>"></i><?=$approved_status;?>
						</a>
					</div>
					<!-- <span class="small badge bg-inverse-<?=$label_color?>">
						<?=$approved_status;?>
					</span> -->
				</td>
				<?php if(User::is_admin()) { ?> 
				...
				<td class="text-right">
					<div class="dropdown">
						...
						<div class="dropdown-menu float-right">
								...
								<a class="dropdown-item" href="javascript:void(0)" onclick="accept_expense(<?=$e->id?>)" title="<?=lang('accept_expense')?>"><i class="fa fa-thumbs-o-up m-r-5" aria-hidden="true"></i><?=lang('approve')?></a>
							
							
								<a class="dropdown-item" href="javascript:void(0)" onclick="decline_expense(<?=$e->id?>)"><i class="fa fa-ban m-r-5" aria-hidden="true"></i> <?=lang('reject')?></a>
							
								<?php if(App::is_permit('menu_expenses','read'))
								{
								?>
									<a class="dropdown-item" href="<?=base_url()?>expenses/view/<?=$e->id?>"><i class="fa fa-eye m-r-5" aria-hidden="true"></i><?=lang('view_expense')?></a>
								<?php 
								}
								if(App::is_permit('menu_expenses','write'))
								{
								?>
								<a class="dropdown-item" href="<?=base_url()?>expenses/edit/<?=$e->id?>" data-toggle="ajaxModal">
									<i class="fa fa-pencil m-r-5"></i><?=lang('edit_expense')?>
								</a>
								<?php 
								} 
								if(App::is_permit('menu_expenses','delete')){?>	 
							
								<a class="dropdown-item" href="<?=base_url()?>expenses/delete/<?=$e->id?>" data-toggle="ajaxModal">
									<i class="fa fa-trash-o m-r-5"></i><?=lang('delete_expense')?>
								</a>
...
<div class="row" id="team_expense">
	...
	<div class="table-responsive">
				...
				if($check_teamlead['is_teamlead'] == 'yes')
				   {
					$expenses = $this->db->query("SELECT * FROM `dgt_expenses` where FIND_IN_SET(".$this->session->userdata('user_id').", expense_approvers) order by id  DESC ")->result();
				}
			...
			<tr id="row_id_<?=$e->id?>" >
				...
				<?php 
					...
					if($check_teamlead['is_teamlead'] == 'yes')
				   {
					...
					<!-- <span class="small badge badge-<?=$label_color?>">
						<?=$approved_status;?>
					</span> -->
				</td>
				<?php if(User::is_admin()) { ?> 
				...
				<td class="text-right">
				   ...
				   { ?> 
								...
								<a class="dropdown-item" href="javascript:void(0)" onclick="accept_expense(<?=$e->id?>)" title="<?=lang('accept_expense')?>"><i class="fa fa-check-circle-o m-r-5" aria-hidden="true"></i><?=lang('approve')?></a>
							
								<a class="dropdown-item" href="javascript:void(0)" onclick="decline_expense(<?=$e->id?>)"><i class="fa fa-minus-circle m-r-5" aria-hidden="true"></i><?=lang('reject')?></a>
						...
						<?php } ?>
							...
							<?php if(User::is_admin() || User::perm_allowed(User::get_id(),'edit_expenses')) { ?>  
								...
								<a class="dropdown-item" href="<?=base_url()?>expenses/edit/<?=$e->id?>" data-toggle="ajaxModal">
									<i class="fa fa-pencil m-r-5" aria-hidden="true"></i><?=lang('edit_expense')?>
								</a>
							
								<?php } if(User::is_admin() || User::perm_allowed(User::get_id(),'delete_expenses')) { ?> 
							
								<a class="dropdown-item" href="<?=base_url()?>expenses/delete/<?=$e->id?>" data-toggle="ajaxModal">
								<i class="fa fa-trash m-r-5" aria-hidden="true"></i>	<?=lang('delete_expense')?>
								</a>
