...
class Expenses extends MX_Controller {
	...
	function __construct()
	{
		...
		if(!App::is_access('menu_expenses'))
		{
...
// 		if(!$this->_can_view_expense($id)){ redirect(); }
			...
			$expense_approvers = $this->db->get('expense_approver_settings')->result_array();
	 	 	...
	 	 	if(!empty($approvers_id)){
				...
				else
			  	{
				  	if(!empty($approvers_id)){
							foreach ($approvers_id as $key => $value) {
								...
								if(!empty($user_details)){
									foreach($user_details as $usedata){
										$approvers[] = $usedata['id'];	
									    $recipient[] = $usedata['email'];
									}
							
								}

								$user_detailsadmin = $this->db->get_where('users',array('designation_id'=>0,'user_type'=>1))->result_array();
								if(!empty($user_detailsadmin)){
									foreach($user_detailsadmin as $admidetail){
										$approvers[] = $admidetail['id'];	
									    $recipient[] = $admidetail['email'];
									}
                ...
                if (count($approvers) > 0) {
                    ...
                    $approve =base_url().'expenses/update_expense_statusmail?id='.$expense_id.'&status=1';
                    $reject =base_url().'expenses/update_expense_statusmail?id='.$expense_id.'&status=2';
	...
	{		
		...
		if( (isset($_POST['id'])&&isset($_POST['status'])) && (!empty($_POST['id'])&&!empty($_POST['status'])) )
		{
			...
			if(!$get_approver_record && !User::is_admin()){
				$afftectedRows = 0;
			}else{
...
// 			$this->db->where('approvers',$this->session->userdata('user_id'));
			...
			print_r(json_encode(array('status'=>$_POST['status'],'updated'=>$afftectedRows,'expense'=>$_POST['id'])));
			exit;
    ...
    public function show_expense()
  	{
  	 ...
  	 public function update_expense_statusmail()
	{		
		...
		{
			$expense_det = $this->db->query("SELECT * FROM dgt_expenses where id = ".$_GET['id']." ")->result_array();
			$acc_det   = $this->db->query("SELECT * FROM `dgt_account_details` where user_id = ".$expense_det[0]['added_by']." ")->result_array();
			$user_det  = $this->db->query("SELECT * FROM `dgt_users` where id = ".$expense_det[0]['added_by']." ")->result_array();
			$approver_det  = $this->db->query("SELECT * FROM `dgt_account_details` where user_id = ".$approver." ")->row_array();

			$approvers_status['status'] = $_GET['status'];
			
			
			$get_approver_record = $this->db->get_where('dgt_expense_approvers',array('expense'=>$_GET['id'],'approvers'=>$approver))->row_array();	
			if(!$get_approver_record && !User::is_admin()){
				$afftectedRows = 0;
			}else{
			
			$this->db->update('dgt_expense_approvers',$approvers_status,array('expense'=>$_GET['id'],'approvers'=>$approver)); 
			$expense_approvers = $this->db->get('expense_approver_settings')->result_array();
			if($expense_approvers[0]['default_expense_approval'] == "seq-approver"){
			// next approvers view
			if($_GET['status'] == 1){				
				...
				$view_status['view_status'] = 1;
				if(!empty($view_next)){
					$this->db->update('dgt_expense_approvers',$view_status,array('expense'=>$_GET['id'],'id'=>$view_next['id'])); 

					$data = array(
								'module' => 'expenses',
								'module_field_id' => $_GET['id'],
								'user' => $view_next['approvers'],
								'activity' => 'Expense Submitted by '.$acc_det[0]['fullname'],
								'icon' => 'fa-plus',
								// 'value1' => $cur.' '.$this->input->post('amount'),
			...
			$expense_approvers_status   = $this->db->get_where('dgt_expense_approvers',array('status !='=>1,'expense'=>$_GET['id']))->num_rows();
			
			$expense_reject_status   = $this->db->get_where('dgt_expense_approvers',array('status'=>2,'expense'=>$_GET['id']))->num_rows();

			$expense_pending_status   = $this->db->get_where('dgt_expense_approvers',array('status'=>0,'expense'=>$_GET['id']))->num_rows();
			
		 	if($expense_approvers_status == 0  ){
		 		$expense_status['expense_status'] = 1;
	         	$this->db->update('dgt_expenses',$expense_status,array('id'=>$_GET['id'])); 
	         	$status = 1;
	         	$email_status = "Approved";
	        ...
	        if($expense_reject_status != 0 ){
 			...
 			if(!empty($acc_det) && !empty($user_det)){
				...
				if(!empty($recipient) && count($recipient) > 0){		 
					...
					$this->session->set_flashdata('tokbox_success', 'Status Updated'); 
			
			/*print_r(json_encode(array('status'=>$_GET['status'],'updated'=>$afftectedRows,'expense'=>$_GET['id'])));
			exit;*/
			if($_GET['status']==1){
                echo "Status Approve Updated";exit;
			}else{
			     echo "Status Reject Updated";exit;
			}
