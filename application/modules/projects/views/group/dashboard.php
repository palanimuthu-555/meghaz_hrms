...
<div class="col-md-12">
			...
			<div class="card-box card-projects">
				...
				<ul class="list-group no-radius m-b-0">
					<li class="list-group-item">
						<span class="float-right text"><?= $info->project_title ?></span>
					...
					<li class="list-group-item">
						<span class="float-right">
							<small class="small">
								...
								?>
								<img src="<?php echo User::avatar_url($user->assigned_user); ?>" class="rounded-circle thumb-xs"
					...
					<li class="list-group-item">
						<span class="float-right">
							<small class="small">
								...
								?>
								<img src="<?php echo User::avatar_url($assign_lead->assign_lead); ?>" class="rounded-circle thumb-xs"
					...
					<li class="list-group-item">
						<span class="float-right">
							...
							class="btn btn-sm btn-success"><i class="fa fa-ellipsis-h"></i></a>
						</span>
							...
							foreach ($activities as $key => $a) {
								...
								<li class="list-group-item">
									<a class="thumb-sm float-left m-r-sm">
										<img src="<?php echo User::avatar_url($a->user); ?>" class="rounded">
									</a>
									<small class="float-right"><?= strftime(config_item('date_format').' %H:%M:%S', strtotime($a->activity_date)) ?></small>
									<strong class="block"><?php echo User::displayName($a->user); ?></strong>
									<span
										class="badge badge-<?= module_color($module) ?>"><?= lang($module) ?></span>
									<small>
										<?php
										if (lang($a->activity) != '') {
											if (!empty($a->value1)) {
	...
	<div class="row row-eq-height">
		...
		<?php if (User::is_admin() || User::is_staff() || Project::setting('show_project_tasks', $project_id)) { ?>
                <div class="col-sm-6 row-eq-height">
                    <div class="card task-panel panel-white panel-eq-tasks">
            ...
            <div class="col-sm-6 row-eq-height">
                <div class="card panel-white task-panel panel-eq-checklist">
					<div class="card-header b-b">
						<div class="row">
							<div class="col-sm-6">
								<h3 class="card-title"><?= lang('todo_list') ?></h3>
							</div>
							<div class="col-sm-6">
								<a href="<?= base_url() ?>projects/todo/add/<?= $project_id ?>" data-toggle="ajaxModal" class="btn btn-primary btn-sm float-right"><?= lang('create_list') ?></a>
							</div>
						</div>
					</div>
                    <div class="card-body">
        ...
        <div class="row  row-eq-height">
            ...
            <?php if (User::is_admin() || User::is_staff() || Project::setting('show_project_files', $project_id)) {
                                    ?>
                <div class="col-sm-6 row-eq-height">
                    <div class="card panel-table panel-eq-files">
                        <div class="card-header">
							<h3 class="card-title"><?= lang('recent_files') ?></h3>
						</div>
                        <div class="slim-scroll" data-height="400" data-disable-fade-out="true" data-distance="0" data-size="5px" data-color="#333333">
            ...
            <?php if (User::is_admin() || User::is_staff() || Project::setting('show_project_bugs', $project_id)) {
				?>
			<div class="col-sm-6 row-eq-height">
				<div class="card panel-table panel-eq-bugs">
					<div class="card-header">
						<h3 class="card-title"><?= lang('recent_bugs') ?></h3>
					</div>
					<div class="slim-scroll" data-height="400" data-disable-fade-out="true" data-distance="0" data-size="5px" data-color="#333333">
						<table class="table table-striped custom-table m-b-0">
							<thead>
								...
								<tr>
									<td>
										<a class="btn btn-sm btn-<?= config_item('theme_color') ?>" href="<?= base_url() ?>projects/view/<?= $project_id ?>/?group=bugs&view=bug&id=<?= $b->bug_id ?>"><?= lang('preview') ?></a>
									</td>
