<section class="card panel-default">
  ...
  ?>
    <header class="card-header panel-h-p1">
      <div class="row">
        <div class="col-sm-12">
    ...
    <div class="card-body">
      ...
      <div class="row">
        <div class="col-lg-6">
          <ul class="list-group no-radius">
            <li class="list-group-item">
              <span class="float-right"><?php echo $m->milestone_name; ?> </span><?=lang('milestone_name')?>
            </li>
            <li class="list-group-item">
              <span class="float-right"><?php echo Project::by_id($m->project)->project_title;?></span>
              <?=lang('project')?>
            </li>
          </ul>
        </div>
        ...
        <div class="col-lg-6">
          <ul class="list-group no-radius">
            <li class="list-group-item">
              <span class="float-right"><?=strftime(config_item('date_format'), strtotime($m->start_date))?></span><?=lang('start_date')?>
            </li>
            <li class="list-group-item">
      ...
      <table id="table-milestone" class="table table-striped table-bordered AppendDataTables">
        ...
        <tbody>
          <?php foreach (Project::milestone_has_tasks($m->id) as $key => $t) {
            ...
            <tr>
              <td><span class="badge badge-<?=$timer?>"><?=$t->timer_status?></span></td>
              ...
              <td>
                <?php  if(User::is_admin() || Project::is_assigned(User::get_id(),$project_id)){ ?>
                <a class="btn btn-sm btn-<?=config_item('theme_color')?>" href="<?=base_url()?>projects/tasks/edit/<?=$t->t_id?>" data-toggle="ajaxModal"><i class="fa fa-edit"></i></a>
                <?php } ?>
