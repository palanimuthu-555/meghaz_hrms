...
class Wiki extends MX_Controller
{
    ...
    function check_wiki_title()
    {
        $wiki_title = $this->input->post('wiki_title');
        $wiki_id = $this->input->post('wiki_id');
        if(!empty($wiki_id)){
            $this->db->where("id !=",$wiki_id);
        }
        $this->db->where("title",$wiki_title);
        $check_wiki_title = $this->db->get('dgt_wiki')->num_rows();
        if($check_wiki_title > 0)
        {
            echo "yes";
        }else{
            echo "no";
