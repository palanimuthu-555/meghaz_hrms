...
class Appuser extends REST_Controller  {
     ...
     public function create_employee_post()
    {
       ...
       if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['username']) && !empty($inputs['password'])&& !empty($inputs['email'])&& !empty($inputs['fullname'])  && !empty($inputs['gender']) && !empty($inputs['phone']) && !empty($inputs['emp_doj'])&& !empty($inputs['reporting_to']) && !empty($inputs['department_id']) && !empty($inputs['designation_id']) && !empty($inputs['user_type'])){
