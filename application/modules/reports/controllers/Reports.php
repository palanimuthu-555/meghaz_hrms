  ...
  // echo "<pre>";print_r($data); exit;
   ...
   // echo "<pre>";print_r($data); exit;
	...
	function _leave_report(){
		...
		if($_POST['pdf']){
			 $html = $this->load->view('pdf/leavereport',$data,true);
		  
		   $file_name = lang('leave_report').'.pdf';
	

		$pdf = array(
			"html"      => $html,
			"title"     => lang('leave_report'),
			"author"    => $this->company_name,
			"creator"   => $this->company_name,
			"badge"     => 'FALSE',
			"filename"  => $file_name
		...
		$this->template
			->set_layout('users')
			->build('report/leave_report',isset($data) ? $data : NULL);
	}

	function leave_report_excel(){
		if(!empty($_POST)){

		$html = $this->load->view('excel/leave_report_excel',$data,true);
