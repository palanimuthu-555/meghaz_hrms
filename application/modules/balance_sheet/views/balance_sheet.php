              ...
              <?php   $start_years = Date('Y', strtotime("-3 Year"));
                 $current_year = date("Y");
                    ...
                    for ($i=$start_years; $i <=$current_year ; $i++) { ?>
                     <li class="nav-item <?php echo ($i == $active_year )?'active':'';?>" id="tab_<?php echo $i;?>"><a data-toggle="tab" class="statement nav-link" data-year="<?php echo $i;?>" href="#statement_<?php echo $i;?>"><?php echo $i;?></a></li>
              <?php }?>
            </ul>
          </div>
                ...
                <table class="table table-hover accounts-table budget-table m-b-0 table-nowrap">
                  ...
                  <tbody id="asset_cont_<?php echo $i;?>">
                    <tr class="title-row">
					  ...
					  if(App::is_permit('menu_balance_sheet','create'))
					  {?>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-sm add-new" id="asset_add" data-year ="<?php echo $i;?>" onclick="asset_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> Add Assets</button></td>
					<?php
					}
					?>	
					</tr>
                            ...
                            foreach ($assets as $asset) {?>
                              <tr>
                                  ...
                                  <td class="text-right actionss">
                                    <a class="btn btn-info btn-sm add"  onclick="save_asset(<?php echo $count?>,<?php echo $i?>,this)" data-type ="asset" data-id ="<?php echo $asset['id'];?>" id="asset_save" title="Add" data-toggle="tooltip"><i class="fa fa-check"></i></a>
                                    <?php
									if(App::is_permit('menu_balance_sheet','write'))
									{?>
									<a class="btn btn-success btn-sm edit" id="asset_edit" title="Edit" data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
									<?php
									}
									if(App::is_permit('menu_balance_sheet','delete'))
									{?>
									<a class="btn btn-danger btn-sm delete" data-toggle="modal" data-target="#asset_delete<?php echo $asset['id']?>" id="" title="Delete" data-toggle="tooltip"><i class="fa fa-trash-o"></i></a>
                                  <?php
									}
								  ?>
								  </td>
                  ...
                  <tbody id="liabilitie_cont<?php echo $i;?>">
                    <tr class="title-row">
					  ...
					  <?php
					  if(App::is_permit('menu_balance_sheet','create')){?>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-sm add-new" id="liabilitie_add" data-year ="<?php echo $i;?>" onclick="liabilitie_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> Add Liabilities</button></td>
					  <?php 
					  } ?>
					</tr>
                    <?php $this->db->select('*');
                           ...
                           // $count = 0;
                            foreach ($liabilities as $liabilitie) {?>
                              <tr>
                                  ...
                                  <td class="text-right actionss">
                                    <a class="btn btn-info btn-sm add"  onclick="save_liabilitie(<?php echo $liabilitie_count?>,<?php echo $i?>,this)" data-type ="liabilitie" data-id ="<?php echo $liabilitie['id'];?>" id="liabilitie_save" title="Add" data-toggle="tooltip"><i class="fa fa-check"></i></a>
                                    <?php
									if(App::is_permit('menu_balance_sheet','write'))
									{?>
									<a class="btn btn-success btn-sm edit" id="liabilitie_edit" title="Edit" data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                    <?php
									}
									if(App::is_permit('menu_balance_sheet','delete'))
									{
									?>
									<a class="btn btn-danger btn-sm delete" data-toggle="modal" data-target="#liabilitie_delete<?php echo $liabilitie['id']?>" id="" title="Delete" data-toggle="tooltip"><i class="fa fa-trash-o"></i></a>
									<?php 
									}
									?>
								  </td>
                  ...
                  <tbody id="shareholder_cont<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong>Shareholder's Equity</strong></td>
						...
						if(App::is_permit('menu_balance_sheet','create'))
						{?>
						<td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-sm add-new" id="shareholder_add" data-year ="<?php echo $i;?>" onclick="shareholder_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> Add Shareholder's Equity</button></td>
						<?php
						}
						?>
					</tr>
                           ...
                           // $count = 0;
                            foreach ($shareholders as $shareholder) {?>
                              <tr>
                                  ...
                                  <td class="text-right actionss">
                                    <a class="btn btn-info btn-sm add"  onclick="save_shareholder(<?php echo $shareholder_count?>,<?php echo $i?>,this)" data-type ="shareholder" data-id ="<?php echo $shareholder['id'];?>" id="shareholder_save" title="Add" data-toggle="tooltip"><i class="fa fa-check"></i></a>
									<?php
									if(App::is_permit('menu_balance_sheet','write'))
									{?>
									<a class="btn btn-success btn-sm edit" id="shareholder_edit" title="Edit" data-toggle="tooltip"><i class="fa fa-pencil"></i></a>
                                    <?php
									}
									if(App::is_permit('menu_balance_sheet','delete'))
									{
									?>
									<a class="btn btn-danger btn-sm delete" data-toggle="modal" data-target="#shareholder_delete<?php echo $shareholder['id']?>" id="" title="Delete" data-toggle="tooltip"><i class="fa fa-trash-o"></i></a>
									<?php
									}?>
								  </td>
                                <tr>
