...
class All_departments extends MX_Controller
{
    public function __construct()
    {
        ...
        //     redirect('');
        // }
		if(!App::is_access('menu_departments'))
		{
    ...
    function departments(){
        ...
        $dept_exits = $this->db->get('departments')->num_rows();
        if($dept_exits>0){
            $this->session->set_flashdata('tokbox_error', lang('department_already_exits'));
    ...
    function edit_designation($des_id)
    {
        if($_POST){
            ...
            $design_exits = $this->db->get('designation')->num_rows();
        if($design_exits>0){
            $this->session->set_flashdata('tokbox_error', 'Designation Already exits');
            redirect($_SERVER['HTTP_REFERER']);
        }
        ...
        if ($_POST) {
            if(isset($_POST['delete_dept']) AND $_POST['delete_dept'] == 'on'){
                ...
                redirect($_SERVER['HTTP_REFERER']);
                    ...
                    $this->db->where('deptname',$this->input->post('deptname'));
                    $dept_exits = $this->db->get('dgt_departments')->num_rows();
                    if($dept_exits>0){
                    $this->session->set_flashdata('tokbox_error', lang('department_already_exits'));
                    redirect($_SERVER['HTTP_REFERER']);
                    }
