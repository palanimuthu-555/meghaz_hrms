...
class Time_sheets extends MX_Controller {
	...
	function __construct()
	{
		...
		if(!App::is_access('menu_timesheets'))
		{
	...
	function index()
	{
		...
		$this->template->title(lang('timesheets').' - '.config_item('company_name'));
		...
		if(!$_POST){
				...
				if(($role_id != 1) && ($role_id != 4))
				{
					...
					//echo "<pre>";print_r($data['all_timesheet']);exit;
					$data['projects'] = $this->db->get('dgt_projects')->result_array();
		...
		// echo print_r($data['projects']); exit();
	...
	{   
		...
		$check_status = $this->timesheet_model->check_timesheetByDate($userid,$tm_date);
		$total_hours = array();
