            <!-- <div class="page-wrapper"> -->
                ...
                <div class="content container-fluid">
					...
					<div class="row">
						...
						<?php if(App::is_permit('menu_timesheets','create'))
						{
						?>
						<div class="col-sm-4 text-right col-7 m-b-30">
							<a href="#" class="btn btn-primary add-btn" data-toggle="modal" data-target="#add_todaywork"><i class="fa fa-plus"></i> Add Today Work</a>
						</div>
					<?php } ?>
					</div>
				</div>
								...
								<table class="table table-striped custom-table m-b-0 AppendDataTables">
									<thead>
										<tr>
											...
											<?php if(App::is_permit('menu_timesheets','write')==true || App::is_permit('menu_timesheets','delete')==true)
											{
											?>
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<?php
												$user_dept=App::get_user_dept_by_id($timesheet['user_id']);
												$dept_name=App::get_dept_by_id($user_dept);
												$user_id=$timesheet['user_id'];
												
       
												$empdata=$this->db->get_where('account_details',array('user_id'=>$user_id))->row_array();
												//echo "<pre>";print_r($empdata);exit;
												 $empname = $empdata['fullname'];
												?>
												<h2 class="table-avatar"><a  href="<?php echo base_url(); ?>employees/profile_view/<?php echo $timesheet['user_id']; ?>" class="avatar"><img class="avatar" src="<?php echo base_url(); ?>assets/avatar/<?php echo $timesheet['avatar']; ?>" alt=""></a><a href="javascript:void(0);"><?php echo $empname; ?>  <span><?php echo $dept_name?$dept_name:''; ?> </span></a></h2>
											</td>
											...
											<td class="text-right" >
												...
												<div class="dropdown">
													...
													<div class="dropdown-menu float-right">
														<?php if(App::is_permit('menu_timesheets','write')){?><a class="dropdown-item" href="#" title="Edit" data-toggle="modal" data-target="#edit_todaywork<?php echo $timesheet['time_id']; ?>"><i class="fa fa-pencil m-r-5"></i> Edit</a><?php }?>
														<?php if(App::is_permit('menu_timesheets','delete')){?><a class="dropdown-item" href="#" title="Delete" data-toggle="modal" data-target="#delete_workdetail<?php echo $timesheet['time_id']; ?>"><i class="fa fa-trash-o m-r-5"></i> Delete</a><?php }?>
													</div>
												</div>
											<?php // }else{ ?>
												<!-- <div class="dropdown">
	     ...
	     <?php if (($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin') || ($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'superadmin')) {  ?>
					...
					<div class="col-lg-4">
						<select class="select2-option form-control" id="user_id" name="user_id"> 
							<option value=""> -- Select User -- </option>
							<?php
							$this->db->select('u.*,a.*');
							$this->db->from('users u');
							$this->db->join('account_details a','u.id=a.user_id');
						...
						<div class="modal-body">
							<form method="post" >
								...
								<div class="row">
									<div class="form-group col-sm-6">
										<label>Date <span class="text-danger">*</span></label>
										<div class="cal-icon"><input class="form-control TimeSheetDate"   value="<?=date('d-m-Y',strtotime($timesheet['timeline_date']))?>"name="timeline_date" id="timeline_date<?php echo $timesheet['time_id']; ?>" type="text" data-date-format="dd-mm-yyyy" ></div>
										<div class="row">
										<div class="col-md-12">
