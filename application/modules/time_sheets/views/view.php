...
<div class="sub-tab m-b-20">
	<ul class="nav nav-tabs nav-tabs-bottom bg-white">
		...
		<li class="nav-item <?php echo ($group == $menu->route) ? 'active' : ''; ?>">
			<a  class="active nav-link" href="<?=base_url()?>projects/view/<?=$p->project_id?>/?group=<?=$menu->route?>">
				<?=lang($menu->name)?>
				<?php if (count($label) > 0) {
				$lab = modules::run($label[0]->route, $p->project_id); ?>
				<span class="badge badge-default float-right"><?php if ($lab > 0) {
				echo $lab;
				} ?></span><?php
				} ?>
