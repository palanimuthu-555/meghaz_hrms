...
class Payroll extends MX_Controller {
    ...
    function index()
    {
		...
		$this->template->title(lang('payroll').' - '.config_item('company_name'));
	...
	{ 
 	...
 	function overtime()
 	{
		...
		if(!App::is_access('menu_overtime'))
		{
			$this->session->set_flashdata('tokbox_error', lang('access_denied'));
             redirect('');
        } 
 	...
 	function settings()
	{
		if(!App::is_access('menu_payrollsettings'))
		{
			$this->session->set_flashdata('tokbox_error', lang('access_denied'));
             redirect('');
        } 
        ...
        $this->session->set_flashdata('tokbox_success', 'Settings Updated Successfully');
