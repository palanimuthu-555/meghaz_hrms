...
class Account extends MX_Controller {
	...
	function account_list(){
        ...
        foreach ($list as $key => $user) {
			...
			$row[] = '<td><span class="badge bg-inverse-'.$borderby.'">'.ucfirst($role_details['role']).'</span></td>';
			$html =  '<td class="text-right">
						<div class="dropdown text-right">
						<a href="javascript:void(0)" class="action-icon" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
							<div class="dropdown-menu float-right">';
														
			// $html .='<a href="'.base_url().'users/account/auth/'.$user['user_id'].'" data-toggle="ajaxModal" title="'.lang("user_edit_login").'"><i class="fa fa-lock"></i> '.lang("user_edit_login").'</a>';

			 if($user['role_id'] == '3') { 

				$html .=  '<a class="dropdown-item" href="'.base_url().'users/account/permissions/'.$user['user_id'].'"  data-toggle="ajaxModal" title="'.lang("staff_permissions").'"><i class="fa fa-shield"></i> '.lang("staff_permissions").'</a>';
			} 
				$html .='<a class="dropdown-item" href="'.base_url().'users/account/update/'.$user['user_id'].'" data-toggle="ajaxModal" title="'.lang("edit").'">
			...
			$html .='<a class="dropdown-item" href="'.base_url().'users/account/ban/'.$user['user_id'].'" class="btn-'.$class.'" data-toggle="ajaxModal" title="'.lang("ban_user").'"><i class="fa fa-times-circle-o"></i> '.lang("ban_user").'</a>';

			$html .='<a class="dropdown-item" href="'.base_url().'users/account/delete/'.$user['user_id'].'" data-toggle="ajaxModal" title="'.lang("delete").'"><i class="fa fa-trash-o"></i> '.lang("delete").'</a>';
