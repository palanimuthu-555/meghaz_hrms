...
class Categories extends MX_Controller
{
    public function __construct()
    {
        ...
        //     redirect('');
        // }
		if(!App::is_access('menu_categories'))
		{
    ...
    function categoiress(){
        ...
        if ($_POST) {
            ...
            $check=$this->db->get_where('budget_subcategory',array('sub_category'=>$sub_category))->num_rows();
            if($check=='0')
            {
            ...
            {
            	 $this->session->set_flashdata('tokbox_error', 'This Sub-Category name is already Exist');
            }
            redirect($_SERVER['HTTP_REFERER']);
        }else{
            // $this->index();
