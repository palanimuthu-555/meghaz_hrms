...
class Tasks extends MX_Controller {
    ...
    function progress(){
        if($_POST){
            ...
            $project_id = $this->db->get_where('tasks',array('t_id'=>$task_id))->row()->project;           
            if(!empty($project_id)){
                $all_task_completeted = $this->db->get_where('tasks',array('project'=>$project_id,'task_progress'=>0))->result();
                $progress = Project::get_progress($project_id);
                if(empty($all_task_completeted)){
                    $project_update = array('status' => 'Done','progress' => 100);
                    App::update('projects',array('project_id'=>$project_id),$project_update);
