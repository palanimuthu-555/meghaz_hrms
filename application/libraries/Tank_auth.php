...
class Tank_auth
{
	...
	*/
	...
	function admin_create_user($username, $email, $password, $fullname, $gender, $company, $city, $state, $address, $pincode, $role, $user_type, $phone, $doj, $email_activation,$designation_id,$department_id,$is_teamlead,$ro)
	{
		// echo $ro; exit;
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
	...
	function forgot_password_candidate($login)
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_cand_login($login))) {
