...
class Assignment extends CI_Model
{
    ...
    static function all()
    {
        return self::$db->order_by('date_created','desc')->get('assignments')->result();
    }

    // Get all pinned projects
   /* static function pinned($type = 'assignments')
    {
        if($type == 'tasks'){
            return self::$db->where('pinned',1)->order_by('t_id','desc')->get('tasks')->result();
        }else{
            return self::$db->where('pinned',1)->order_by('project_id','desc')->get('assignments')->result();
        }
    }*/
    ...
    {
        return self::$db->where('project_id',$id)->get('assignments')->row();
    }
    ...
    static function update($project,$data){
        return self::$db->where('project_id',$project)->update('assignments',$data);
    }


    // Delete project team
    static function delete_team($project){
        return self::$db->where('project_assigned',$project)->delete('assign_assignments');
    }
    ...
    static function by_where($array = NULL,$join = NULL){
        ...
        self::$db->order_by('project_id',DESC);
        return self::$db->where($array)->get('assignments')->result();
    }

    // Get uncompleted projects
    static function get_uncomplete($limit = NULL)
    {
        return self::$db->where('status !=','Done')->order_by('project_id','DESC')->get('assignments',$limit)->result();
    }

    // Get project assigned users
    static function project_team($project)
    {
        return self::$db->where('project_assigned',$project)->get('assign_assignments')->result();
    }

    static function by_client($company)
    {
        return self::$db->where('client',$company)->get('assignments')->result();
    }
    ...
    static function is_assigned($user, $project){
        $assigned = self::$db->where(array('assigned_user'=>$user,'project_assigned' => $project))->get('assign_assignments')->num_rows();
    ...
    static function activity($id){
        $modules = array('milestones', 'tasks', 'files','bugs','assignments','links');
    ...
    static function convert_estimate_to_project($estimate){
        ...
        self::$db->insert('assign_assignments',$data);
    ...
    static function delete($project){
        ...
        self::$db->where(array('module' => 'assignments','module_field_id' => $project))->delete('activities');
        ...
        self::$db->where(array('project_id' => $project))->delete('assignments');
    }

    // Get staff based project
    static function staff_project($staff_id)
    {
        return self::$db->like('assign_to',$staff_id)->get('assignments')->result();
    }
