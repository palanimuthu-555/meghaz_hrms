	...
	*/
	...
	static function approve_candidate($user_id = NULL){
	    ...
	    elseif ($user_id==1) {	    
			...
			$query = "SELECT ofs.*,ofa.status as approver_status,ofa.id as app_row_id from dgt_offers as ofs LEFT JOIN dgt_offer_approvers as ofa on ofs.id = ofa.offer  GROUP by ofs.id  ORDER BY ofs.id DESC";
			return self::$db->query($query)->result();
